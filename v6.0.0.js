...
export default {
	async fetch(request, env, ctx) {
		...
		if (Router.isWebSocketUpgrade(request) && url.pathname === "/LOWKEY_PANEL") {
...
const Router = {
	...
	async handleApi(request, url, env, ctx) {
		...
		if (url.pathname === "/api/update-panel" && request.method === "POST") {
			return new Response(JSON.stringify({ error: "قابلیت آپدیت خودکار از سورس بیرونی در این پنل غیرفعال شده است." }), { status: 400, headers: { "Content-Type": "application/json" } });
		}
		if (url.pathname === "/api/restart-core" && request.method === "POST") {
			return new Response(JSON.stringify({ error: "قابلیت ری‌استارت خودکار از سورس بیرونی در این پنل غیرفعال شده است." }), { status: 400, headers: { "Content-Type": "application/json" } });
		}
...
const SubscriptionService = {
	async generateText(user, host) {
		...
		const m2 = "🚀@lowkey878 سرعت رایگان🚀";
		links.push(atob("dmxlc3M6Ly8=") + user.uuid + "@0.0.0.0:1?encryption=none&security=none&type=ws&host=" + host + "&path=%2FLOWKEY_PANEL#" + encodeURIComponent(m1));
		links.push(atob("dmxlc3M6Ly8=") + user.uuid + "@0.0.0.0:1?encryption=none&security=none&type=ws&host=" + host + "&path=%2FLOWKEY_PANEL#" + encodeURIComponent(m2));
		...
		links.push(atob("dmxlc3M6Ly8=") + user.uuid + "@" + host + ":80?path=%2FLOWKEY_PANEL&security=none&encryption=none&host=" + host + "&fp=" + fp + "&type=ws#" + encodeURIComponent(infoRemark));
		ips.forEach((ip) => {
			ports.forEach((portStr) => {
				...
				links.push(atob("dmxlc3M6Ly8=") + user.uuid + "@" + ip + ":" + portStr + "?path=%2FLOWKEY_PANEL&security=" + tlsVal + "&encryption=none&insecure=0&host=" + host + "&fp=" + fp + "&type=ws&allowInsecure=0&sni=" + host + userFrag + "#" + encodeURIComponent(remark));
			});
		});
...
<head>
            ...
            theme: {
                extend: {
                    ...
                    colors: {
                        ...
                        brand: { from: '#7c3aed', via: '#c026d3', to: '#f97316' }
                    }
                }
            }
        }
    ...
    <style>
        ...
        * {
        ...
        .brand-gradient-bg {
            background: linear-gradient(160deg, #7c3aed 0%, #c026d3 55%, #f97316 100%);
        }
        #sidebar { scrollbar-width: thin; }
        .sidebar-link.active {
            background: linear-gradient(90deg, rgba(168,85,247,0.18), rgba(236,72,153,0.18));
            border-color: rgba(217,70,239,0.4) !important;
            color: #c026d3 !important;
        }
        .dark .sidebar-link.active { color: #e879f9 !important; }
    </style>
</head>
<body class="bg-gray-50 text-gray-900 dark:bg-amoled-bg dark:text-zinc-100 min-h-screen transition-colors duration-200 md:flex md:flex-row-reverse">
    <aside id="sidebar" class="md:w-64 md:min-h-screen md:sticky md:top-0 md:overflow-y-auto flex-shrink-0 border-b md:border-b-0 md:border-s border-gray-200 dark:border-amoled-border bg-white dark:bg-amoled-card">
        <div class="brand-gradient-bg px-5 py-5 flex items-center gap-3">
            <div class="w-9 h-9 rounded-xl bg-white/20 backdrop-blur flex items-center justify-center text-lg font-black text-white shadow-inner flex-shrink-0">L</div>
            <div class="min-w-0">
                <h1 class="text-base font-black text-white leading-tight truncate" dir="ltr">Lowkey Panel</h1>
                <span id="panel-version" class="text-[10px] px-1.5 py-0.5 font-bold bg-white/25 text-white rounded-full inline-block mt-0.5">v1.5.10</span>
            </div>
        </div>
        <div class="p-3 flex md:flex-col flex-row flex-wrap gap-2">
            <a href="https://t.me/lowkey878" target="_blank" rel="noopener noreferrer" class="sidebar-link flex items-center gap-2.5 px-3 py-2.5 rounded-xl border border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900/50 hover:bg-sky-50 dark:hover:bg-sky-950/30 text-gray-700 dark:text-zinc-300 hover:text-sky-500 transition-all duration-200 text-sm font-bold flex-1 md:flex-none" title="Telegram">
                <svg class="w-5 h-5 flex-shrink-0 text-sky-500" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm4.64 6.8c-.15 1.58-.8 5.42-1.13 7.19-.14.75-.42 1-.68 1.03-.58.05-1.02-.38-1.58-.75-.88-.58-1.38-.94-2.23-1.5-.99-.65-.35-1.01.22-1.59.15-.15 2.71-2.48 2.76-2.69a.2.2 0 00-.05-.18c-.06-.05-.14-.03-.21-.02-.09.02-1.49.94-4.22 2.79-.4.27-.76.41-1.08.4-.36-.01-1.04-.2-1.55-.37-.63-.2-1.12-.31-1.08-.66.02-.18.27-.36.74-.55 2.92-1.27 4.86-2.11 5.83-2.51 2.78-1.16 3.35-1.36 3.73-1.37.08 0 .27.02.39.12.1.08.13.19.14.27-.01.06.01.24 0 .24z"/>
                </svg>
                <span class="truncate">پشتیبانی تلگرام</span>
            </a>
            <button id="theme-toggle" 
                    class="sidebar-link flex items-center gap-2.5 px-3 py-2.5 rounded-xl border border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900/50 hover:bg-amber-50 dark:hover:bg-amber-950/30 text-gray-700 dark:text-zinc-300 hover:text-amber-500 transition-all duration-200 text-sm font-bold flex-1 md:flex-none"
                    title="تغییر تم">
                <svg id="sun-icon" class="w-5 h-5 hidden dark:block flex-shrink-0 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <svg id="moon-icon" class="w-5 h-5 block dark:hidden flex-shrink-0 text-amber-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"></path>
                </svg>
                <span class="truncate">تغییر تم</span>
            </button>
            <button onclick="toggleSettingsModal(true)" 
                    class="sidebar-link flex items-center gap-2.5 px-3 py-2.5 rounded-xl border border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900/50 hover:bg-fuchsia-50 dark:hover:bg-fuchsia-950/30 text-gray-700 dark:text-zinc-300 hover:text-fuchsia-500 transition-all duration-200 text-sm font-bold flex-1 md:flex-none"
                    title="تنظیمات">
                ...
                <span class="truncate">تنظیمات</span>
            </button>
            <button 
                onclick="logoutAdmin()" 
                class="sidebar-link flex items-center gap-2.5 px-3 py-2.5 rounded-xl border border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900/50 hover:bg-red-50 dark:hover:bg-red-950/30 text-gray-700 dark:text-zinc-300 hover:text-red-500 transition-all duration-200 text-sm font-bold flex-1 md:flex-none"
                title="خروج">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <main class="flex-1 min-w-0 max-w-6xl mx-auto px-4 py-8 pb-56 md:pb-32 relative">
<div class="pointer-events-none fixed inset-0 -z-10 opacity-[0.06] dark:opacity-[0.12]" style="background: radial-gradient(circle at 15% 0%, #7c3aed, transparent 40%), radial-gradient(circle at 85% 20%, #ec4899, transparent 40%), radial-gradient(circle at 50% 100%, #f97316, transparent 40%);"></div>
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
    ...
    <div class="bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-xl p-2.5 shadow-sm flex flex-col justify-center gap-1 hover:shadow-md hover:border-blue-400 dark:hover:border-blue-500/50 transition duration-300 relative overflow-hidden group min-h-[64px]">
		...
		<div class="flex items-center justify-between mb-4">
			...
			<button onclick="openCreateModal()" class="p-2.5 rounded-xl bg-gradient-to-tr from-violet-600 via-fuchsia-600 to-orange-500 border-2 border-transparent hover:brightness-110 transition-all duration-300 text-white shadow-md shadow-fuchsia-500/30 hover:shadow-lg hover:shadow-fuchsia-500/40 hover:scale-105">
                ...
                <div class="space-y-2.5">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="input-name" placeholder="PANEL_LOWKEY" maxlength="32" class="w-full pl-3 pr-9 py-1.5 bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/50 text-xs font-semibold text-gray-800 dark:text-zinc-100 placeholder-gray-400/80 transition" required>
                        </div>
                    </div>
...
function getVlessLink(username) {
            ...
            const m2 = '♨️ @lowkey878 سرعت رایگان ♨️';
            links.push('vle' + 'ss://' + (user.uuid || '') + '@0.0.0.0:1?encryption=none&security=none&type=ws&host=' + host + '&path=%2FLOWKEY_PANEL#' + encodeURIComponent(m1));
            links.push('vle' + 'ss://' + (user.uuid || '') + '@0.0.0.0:1?encryption=none&security=none&type=ws&host=' + host + '&path=%2FLOWKEY_PANEL#' + encodeURIComponent(m2));
            ips.forEach((ip) => {
                ports.forEach((portStr) => {
                    const isTlsPort = tlsPorts.includes(portStr);
                    const tlsVal = isTlsPort ? 'tls' : 'none';
                    const remark = user.username + ' | \u200E' + ip + ' | \u200E' + portStr;
                    links.push('vle' + 'ss://' + (user.uuid || '') + '@' + ip + ':' + portStr + '?path=%2FLOWKEY_PANEL&security=' + tlsVal + '&encryption=none&insecure=0&host=' + host + '&fp=' + fp + '&type=ws&allowInsecure=0&sni=' + host + userFrag + '#' + encodeURIComponent(remark));
                });
            });
            return links.join('\\n');
        }
...
document.addEventListener('DOMContentLoaded', () => {
            ...
            window.changeRefreshRate = function(val) {
                ...
                localStorage.setItem('lowkey_refresh_rate', ms);
                window.startRefreshInterval(ms);
                showToast('نرخ رفرش پنل تغییر کرد');
            };

            const savedRate = localStorage.getItem('lowkey_refresh_rate');
        ...
        function getVlessLink() {
            ...
            ips.forEach(function(ip, ipIndex) {
                ports.forEach(function(portStr) {
                    ...
                    links.push('vle' + 'ss://' + (u.uuid || '') + '@' + ip + ':' + portStr + '?path=%2FLOWKEY_PANEL&security=' + tlsVal + '&encryption=none&insecure=0&host=' + host + '&fp=' + fp + '&type=ws&allowInsecure=0&sni=' + host + userFrag + '#' + encodeURIComponent(remark));
                });
            });
            return links.join('\\n');
        }
